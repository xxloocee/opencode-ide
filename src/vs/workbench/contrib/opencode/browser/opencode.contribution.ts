...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	...
	properties: {
		...
		[SessionsOpenCodeCwdSettingId]: {
			...
			default: '',
			scope: ConfigurationScope.APPLICATION,
			description: localize('openCode.cwd', "Optional working directory used to launch the OpenCode server command. When empty, Ergouzi IDE falls back to the current window workspace root.")
		},
		[SessionsOpenCodeUiPackageSettingId]: {
			type: 'string',
			default: 'app-ide',
		...
		[SessionsOpenCodeEnableGenerativeUiCspSettingId]: {
			...
			default: true,
			scope: ConfigurationScope.APPLICATION,
			description: localize('openCode.enableGenerativeUiCsp', "When enabled, Ergouzi IDE injects OPENCODE_ENABLE_GENERATIVE_UI_CSP=1 for app-ide launches so generative widgets can use the IDE-specific CSP policy.")
		},
	}
});
